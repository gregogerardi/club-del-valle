...
export class CarouselComponent {
  ...
  @Input() set selectedBackgroundContext(selectedBackgroundContext) {
    ...
    this.imagePath = interval(5000).pipe(
      startWith(0),
      tap((i) =>
        this.injectLinkPrefetchIn4g(
          `assets/background/${this._selectedBackgroundContext}/${
            ((i + 1) %
              BACKGROUND_CONTEXTS[this._selectedBackgroundContext]
                .numberOfImages) +
            1
          }.jpg`
        )
      ...
      map(
        (i) =>
          `url('./assets/background/${this._selectedBackgroundContext}/${
            (i %
              BACKGROUND_CONTEXTS[this._selectedBackgroundContext]
                .numberOfImages) +
            1
  ...
  public imagePath = interval(5000).pipe(
    startWith(0),
    map(
      (i) =>
        `url('./assets/background/${this._selectedBackgroundContext}/${
